...
以下はClaudeが自己判断するためのガイドライン（スクリプトはキーワード検知のみ行う）:
...
Claude Code を再起動して、任意の返答をさせる。Claude の返答後に【逃げちゃダメだチェック】の内容が Claude のコンテキストに注入されれば導入成功。

## 注意

- このフックは **毎ターン発動** する。先送りがない場合は「無視してよい」と明記しているため、Claude は自動スルーする。
- `async` は **設定しない**。Claude がリアルタイムで読む必要があるため。（`settings.json` の Hook エントリに `"async": true` を付けないこと）
