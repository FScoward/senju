...
> **実行環境メモ（Claude / Codex）**
>
> この文書の `Skill(...)`, `Agent(...)`, `TeamCreate`, `SendMessage` は Claude Code の表記を基準にしている。
> Codex で実行する場合は SKILL.md の「実行環境互換」に従い、該当スキルの `SKILL.md` 実行、sub-agent / multi-agent tool、または `.claude/tmp` 配下の file-handoff に読み替える。
> `.claude/tmp` は互換 artifact namespace であり、Codex 実行時も同じパスに保存する。
...
**→ `refine-ticket` スキルを呼び出して、チケットの AC を磨く。**

```
Skill(refine-ticket)
# 入力: チケットID/内容、Tier（E1 で判定済みなら渡す。直接実行なら refine-ticket が自前判定）
#       出力ファイル = scratch.md（または上位フロー経由なら spec.md の該当チケット分）
```

`refine-ticket` が以下を実行する（詳細は当該スキルの SKILL.md / references を参照）:
- チケット内容の充足性確認と、不十分時の親チケット/Epic 参照（1回で打ち切り）
- GWT+Examples フォーマットへのリライト（抽象表現→具体値）
- 7項目チェックリスト（具体性・テスト可能性・正常系・異常系・境界値・副作用・非機能）での過不足分析
- 不足項目の補完（`[補完]` マーク + 補完履歴ログ出力）

> 上位フロー E1 を経由している場合は `spec.md` / `spec-draft.md` に磨き込み済みの AC があるか先に確認し、あれば refine-ticket の再実行は不要（下記スキップ条件）。
...
### DR セルフチェック（T0 末尾必須・30 秒）

`refine-ticket` が出力した**補完履歴ログを読み**、以下に該当するものを未起票 DR として作成してから T1 へ進む。DR 化は kouunryuusui 側の責務（refine-ticket は履歴を出力するだけ）。

- [ ] AC を補完・解釈・分割した（`[補完]` マーク付きは特に対象）
- [ ] AC に書かれていないが必要と判断した前提条件・制約を追加した
- [ ] 親チケット参照で得た情報を spec として採用した
- [ ] 修正時の期待動作・回帰基準を新たに定義した

検知したものは種別（明示外 / 妥協 / 先送り / スコープ追加）を付けて DR-NNN として `.claude/tmp/decisions/` に記録する。
...
### Team / file-handoff ワーカーモード（E4から呼ばれる場合）

上位フロー E4 のチケット実行ループから呼ばれる場合、**T1〜T5の全工程をワーカー単位で実行する**。
これにより、メインセッション（Team Lead）は E4 のオーケストレーションに専念し、各チケットの実装はワーカーエージェントとして並列進行する。

> **設計思想**: Claude では Native Team の `SendMessage` によるリアルタイムメッセージングを使用する。Codex で同等機構が無い場合は `.claude/tmp/team/` と `.claude/tmp/handoffs/` のファイルで進捗共有・コンフリクト予防・段階的エスカレーションを行う。

| 項目 | 値 |
|------|-----|
| 起動方法 | Claude: `Agent(team_name="rdf-{EPIC-ID}", name="worker-{チケットID}", subagent_type="general-purpose")` / Codex: 利用可能な sub-agent、無ければ順次実行 |
| 作業ディレクトリ | worktreeルート |
| 進捗通知 | Claude: `SendMessage` / Codex: `.claude/tmp/team/{team}/events.log` へ追記 |
| 完了通知 | Claude: `SendMessage("DONE: {チケットID} {status} ...")` / Codex: `DONE:` 行を events.log に追記 |
| 結果収集 | Team Lead がメッセージまたは events.log / handoff artifact から収集 |

#### 通信プロトコル

ワーカーは以下のタイミングで `SendMessage` または events.log 追記を使って Team Lead に報告する:
...
Codex で Team 機構が使えない場合は、BE → FE → 統合確認の順で実行し、並列化できなかった理由と実行順を `scratch.md` に残す。並列実行の有無で品質基準は下げない。
...
### 実装中の説明責任（`code-why` スキル）

`code-why` がインストールされている場合、T3 の実装中に発動して判断根拠（WHY）を記録する。説明の深さは変更の重さに比例させる（rename / typo / import 追加は省略、設計選択を伴う実装はフルセット）。

- **実装前**: アルゴリズム・データ構造・設計パターンの選択を伴う場合のみ、方針宣言（採用理由＋退けた代替案）を会話に残す
- **実装後**: 非自明な判断に `[Why-候補] L{行番号}: {1 行の WHY}` を会話に書き残す
- **コミット時**: 会話の `[Why-候補]` を収集し、コミットメッセージ末尾の `Why:` セクションに 1 行ずつまとめる（`references/tdd-quality.md` のコミット粒度ガイドライン参照）
- **大レベルへ昇格した判断**（spec / AC からの逸脱・妥協・先送り・スコープ追加）は会話だけで流さず、下記 DR セルフチェックを経て DR-NNN に一本化する

責任境界の詳細（行/関数レベル WHY = code-why、ステージレベル判断 = DR）は `code-why` スキルの SKILL.md を参照。kouunryuusui 側では重複定義しない。

### DR セルフチェック（T3 末尾必須・QG 起動前・30 秒）

実装中に以下が発生しなかったか自己確認。あれば未起票 DR を作成してから QG へ進む。

- [ ] AC に書かれていない実装判断（アルゴリズム選択、データ構造、エラーハンドリング方針）をした
- [ ] spec から逸脱・妥協した（性能・互換性・運用都合での代替実装）
- [ ] TODO / FIXME / `// 後で対応` を残した
- [ ] AC 外の追加修正（リファクタ・命名変更・周辺コード改善）を含めた
- [ ] code-why で「大」レベルと判定された判断がある（→ 重複させず DR に一本化）

検知したものは種別を付けて DR-NNN として `.claude/tmp/decisions/` に記録する。trivial 除外基準（rename / typo / 単純踏襲）に該当するものは不要。
...
| 呼び出し方 | 内容 |
|----------|------|
| スキル起動 | Claude: `Skill(skill="simplify", args="<PR差分の指定>")` / Codex: `simplify` の `SKILL.md` を読み、同じ args 相当の範囲で実行 |
...
base ブランチを検出してから args を組み立てて呼び出す。Codex ではこのテンプレートの args を `simplify` スキル実行時の入力として扱う:
...
**呼び出し方法**: `mihari` スキルを起動し、以下の入力を渡す。Codex では `mihari` の `SKILL.md` と references を読み、この入力契約で実行する:
...
2. `advisor()` を呼び出す（Codex では `codex-advisor` または同等のセカンドオピニオン手段）
3. advisor の応答を確認:
...
**Claude の Skill 呼び出し、または Codex で該当 `SKILL.md` を実行したログを行として記録する**こと。
...
| QG-2 simplify | PASS | `Skill(simplify)` / Codex skill execution no further changes（または修正 N 件適用済み） |
...
`mihari` / `review-loop` の PASS 行は**実際にスキルを起動した結果**を書く。Codex では該当 `SKILL.md` のプロトコルを実行した結果を書く。
...
| 過剰実装がないか（スコープ外変更） | `git diff --stat` で変更ファイルを確認 | 不要な変更を削除（残す場合は DR-NNN を起票） |
...
| **spec 外決定が DR として漏れていないか** | `git diff` を読み返し、spec/AC に書かれていない判断・妥協・先送り・追加変更が DR 化されているか確認 | 未起票 DR を作成（種別: 明示外 / 妥協 / 先送り / スコープ追加） |
...
### Team / file-handoff ワーカーとして実行されている場合（E4経由）

Team / file-handoff ワーカーとして実行されている場合、T5は**pushせずに停止**する。**DONEメッセージ送信前に、ハンドオフアーティファクトを書き出す**:
...
その後、`SendMessage` または `.claude/tmp/team/{team}/events.log` への `DONE:` 追記で Team Lead に完了を報告する:
...
Push と PR 作成はメインセッション（E4 Team Lead）の一括承認フローで実行される。ワーカーはメッセージ送信または events.log 追記後、Team Lead からの指示を待つ（承認 → push実行、または追加修正指示）。
