...
> **実行環境メモ（Claude / Codex）**
>
> この文書の `Skill(...)`, `Agent(...)`, `TeamCreate`, `TaskCreate`, `SendMessage`, `advisor()` は Claude Code の表記を基準にしている。
...
3. **A.C.の品質チェック・補完・生成は `refine-ticket` スキルに委譲する**:

```
Skill(refine-ticket)
# 入力: チケットID/内容、Tier（上記タスク性質判定の結果を渡す）、出力ファイル = spec-draft.md
```

`refine-ticket` が以下を実行する（詳細は当該スキルの SKILL.md / references を参照）:
- 存在チェック（AC有無 / 仕様書リンク有無 / 内容充足性）と、内容不十分時の周辺チケット参照（親チケット/Epic を1回だけ参照）
- 仕様書/PRD からの US+AC 自動生成（AC が無い場合。`sier-dev` での要件定義書生成にも対応）
- GWT+Examples フォーマットへのリライトと 7項目過不足分析（具体性・テスト可能性・正常系・異常系・境界値・副作用・非機能）
- 不足項目の補完（`[補完]` マーク + 補完履歴ログ出力）
- 結果を `spec-draft.md` に品質レポート形式で出力

### 各チケットのAC生成（SBE形式）

E2の設計と E3の分割結果に基づき、各チケットの受け入れ条件を生成する。

**→ `refine-ticket` スキルを呼び出し、出力先に `spec.md` の該当チケット分を指定する。** AC品質基準（GWT+Examples・7項目チェックリスト）は refine-ticket が担保する。E1で補完済みのACがある場合はそれをベースにし、E2の設計情報で肉付けする。
...
### DR セルフチェック（E1 末尾必須・全 Tier 対象・30 秒）

E1 は **spec 外決定の最大の発生源**（AC 補完・推定・親チケット参照）。SSOT 宣言する前に、以下が無かったか自己確認し、検知したものは未起票 DR を作成する。

- [ ] `[補完]` マーク付き AC を追加した（→ 種別: 明示外、各 AC につき 1 DR を原則）
- [ ] チケット本文に書かれていない前提・制約・非機能要件を spec-draft に含めた
- [ ] 親チケット / Epic 参照で得た情報を spec として採用した
- [ ] ドメイン知識・コードベース調査から推定で AC を埋めた
- [ ] AC を分割・統合・順序変更した

**「E1 末尾: spec 外決定 N 件（0 を含む数値）」を必ず会話で明言する。**「該当なし」だけで素通りしない。0 件と数値で言うことで「気付かなかった」と「無かった」を分離する。
...
> **必須読み込み: `references/impact-analysis.md`**
> このフェーズの観点抜けがレビュー指摘・回帰の主要因。各 Agent に渡す
> プロンプトは impact-analysis.md の **7観点チェックリスト** を必ず含める。

`Explore` (haiku) で並列実行。各エージェントは impact-analysis.md の
担当観点を **明示的に潰す** こと（「該当なし」も書く）。

- **Agent A: バックエンド**（Domain, UseCase, Infrastructure, Presentation層）
  主担当観点: 1 呼び出し元 / 2 呼び出し先 / 6 横断的関心事
- **Agent B: フロントエンド**（コンポーネント、hooks、型定義、API呼び出し）
  主担当観点: 1 呼び出し元 / 4 契約・型・スキーマ
- **Agent C: DB・設定・バッチ・インフラ**（マイグレーション、設定ファイル、テスト、Terraform、Cloud Scheduler）
  主担当観点: 3 データフロー / 7 リリース順序・互換性

**共通観点**: 観点 5（既存テストの fallout）は **各 Agent が自分の領域で必ず**列挙する。

**統合**: 3 Agent の出力を統合し、impact-analysis.md の「出力フォーマット」に従って
`design.md` 冒頭の **影響範囲レポート** セクションに書き出す。

> **既存修正時の鉄則**: 「既に実装済み → 影響範囲調査は軽くて良い」は誤り。
> 既存修正こそ波及が広いため、7観点を必ず潰す（impact-analysis.md 参照）。

> **refine-ticket からの引き継ぎ**: E1 で `refine-ticket` が `spec-draft.md` に
> 軽量影響範囲レポートを書いていることがある。E2a はその深堀拡張版として動く。
> 軽量版で「該当なし」だった観点も E2a では再確認すること（深度が違うため見えるものが増える）。
...
- **受け入れ条件は `refine-ticket` が磨いた GWT+Examples フォーマットのまま記述すること**
...
`ticket-plan.md` の依存グラフに従い、各チケットに対して**下位フロー（T1〜T5）を Team / sub-agent / file-handoff で実行**する。

> **設計思想**: Claude では claude-peers-mcp のセッション間コミュニケーション概念を Claude Code Native Team（`TeamCreate` / `SendMessage` / `TaskUpdate`）で実現する。Codex で同等機構が無い場合は `.claude/tmp/team/` の events log と `.claude/tmp/handoffs/` の artifact で進捗共有・コンフリクト予防・段階的エスカレーションを行う。

### 実行ルール

1. **依存関係を尊重**: 依存先チケットのタスクが `completed` になってから次のチケットに着手
2. **並列可能なチケットは並列実行**: 依存関係がないチケット同士は、利用可能なら Team / sub-agent ワーカーとして同時起動。Codex で並列機構が無い場合は順次実行し、順次化した理由を `scratch.md` と events log に残す
3. **進捗共有**: 各ワーカーは `SendMessage` または `.claude/tmp/team/{team}/events.log` で進捗・ブロッカー・変更ファイル情報を親に通知
...
### File-handoff セットアップ（Codex fallback）

Codex で Team 系 tool が使えない場合は、以下のファイルを作って同じ状態管理を行う:

```text
.claude/tmp/team/rdf-{EPIC-ID}/
  tasks/{チケットID}.md     # status, dependencies, planned_files, branch
  events.log                # PROGRESS / FILES / BLOCKED / ESCALATE / DONE
.claude/tmp/handoffs/
  {チケットID}.md           # 下位フロー T5 完了時のハンドオフ
```

`events.log` の行形式は `references/subagent-protocol.md` のステータスメッセージと同じにする。
...
│
├─ TeamCreate("rdf-{EPIC-ID}") または .claude/tmp/team/rdf-{EPIC-ID}/ 作成
├─ 依存グラフから全チケットを TaskCreate または tasks/*.md で登録
├─ 依存なしチケットを Agent(team_name, general-purpose) または Codex sub-agent / 順次実行で起動
├─ SendMessage または events.log で各ワーカーの進捗を受信
...
│  ├─ T1〜T5 を自律実行
│  ├─ 進捗を SendMessage または events.log で親に通知（フェーズ移行ごと）
│  ├─ 変更ファイルリストを SendMessage または events.log で共有（コンフリクト予防）
│  └─ T5完了 → SendMessage / events.log に "DONE: ... awaiting_approval"
...
依存関係が解消されたチケットを、Claude では `Agent` で起動する:
...
Agent(
  ...
  prompt="""
    ...
    **→ `references/subagent-protocol.md` を参照すること。**
    ...
    以下のタイミングで SendMessage（Codex fallback では events.log）を使って team-lead に報告すること:
...
Codex では、同じ prompt 内容を sub-agent / multi-agent tool に渡す。利用不可の場合はメインセッションで下位フローを順次実行し、開始時と完了時に `events.log` へ `PROGRESS:` / `DONE:` を記録する。
...
## DR セルフチェック（E2 末尾必須・30 秒）

設計フェーズで以下が無かったか自己確認。あれば未起票 DR を作成してから次工程へ進む。

- [ ] spec / AC に書かれていないテーブル・カラム・API・パラメータを設計に含めた
- [ ] 既存実装パターンと整合させるため AC の表現を再解釈・補完した
- [ ] 性能・互換性・運用の理由で「理想解」から妥協した
- [ ] スコープを縮小・分割・先送り（TODO/FIXME 化）した
- [ ] AC 外の追加変更（リファクタ・命名変更など）を含めた
...
メインセッション（Team Lead）は各ワーカーからの `SendMessage` または events.log を受信し、以下のように処理する:

| メッセージプレフィクス | 処理 |
|----------------------|------|
| `PROGRESS:` | `ticket-plan.md` のチケット状態を更新。ログに記録 |
| `FILES:` | 変更ファイルリストを記録。他ワーカーとの競合をチェック |
| `BLOCKED:` | 他ワーカーの状況を確認し、解決策を `SendMessage` または events.log / task file で返信 |
...
QG（品質ゲート）で詰まったワーカーは、ユーザーにエスカレーションする前に**親セッションに相談**する。Codex fallback では同じ内容を events.log に `ESCALATE:` として追記する:
...
3. `.claude/memory/learnings.md` に知見を記録（任意。Codex 実行時も互換パスとして扱う）
...
7. 最終報告書をユーザーに表示
8. Team シャットダウン（Claude: `SendMessage` で各ワーカーに `shutdown_request` → `TeamDelete` / Codex fallback: events.log に `shutdown_request` を記録して tasks を closed にする）
