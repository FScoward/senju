...
├── 05_acceptance-tests/       ← このスキルが生成（Step 5）
│   ├── AT-overview.md         ← テストサマリー + 機能→TCマッピング表
│   └── features/              ← Tier 2/3 のみ
│       ├── AT-feature-001-xxx.md
│       └── AT-feature-002-yyy.md
└── 06_wbs/
    ├── WBS.md                 ← このスキルが生成
    ├── stacked-pr-plan.md     ← このスキルが生成（Step 4c）
...
Tierと要件の把握ができたら、**So What? / Why So? で要件を深掘りする**：

収集した要件を1つずつ問い直し、表層的な機能要求の奥にある本当のビジネス価値を確認する。
問い方の詳細・良し悪しの対比例は `references/so-what-why-so.md` を参照。
...
2往復して「それでもまだ曖昧」な要件は、その場でユーザーに確認する（先送りしない）。
...
**各設計判断で So What? / Why So? を往復させる**：

判断を1つ下すたびに以下の問いを回す。答えを設計方針書と DR に記録することで、後続ドキュメントとの整合性チェックが自動化される。問い方の詳細・対比例は `references/so-what-why-so.md` を参照。

```
So What?  この設計判断で、ユーザー・開発者・運用チームに何が具体的に変わるか？
           （プラス面と犠牲にするものを両方書く）
Why So?   なぜ要件・制約がこの判断を要求するか？
           （要件のどの記述がこれを支えているか。他案では何が満たせないか）
```

「犠牲にするものが書けない」場合は見落としのサイン。再度問い直す。
...
#### Step 4c: Stacked PR 計画の生成

> **Tier 1（P3チケット3件以下）はこのステップをスキップしてよい。** 依存なしなら単一PRで完結させ、依存がある場合のみ2〜3本の浅いスタックを作る。

Step 4b で生成したチケットドラフトの **「依存チケット」フィールド**を読み取り、Stacked PR の構成計画を立てる。
実行手順の詳細（rebase・force-with-lease・merge 順序など）は `/stacked-pr` スキルに委ねる。このステップは**計画**のみを生成する。

**DAG → スタック変換の手順**：

1. 全チケットの「依存チケット」フィールドを収集し、有向グラフ（DAG）として整理する
2. チェーン単位でグループ化し、トポロジカルソートで実装順を決定する
   - 依存関係のない独立チケットは**スタンドアローンPR**として扱い、スタックには含めない
   - チェーンの深さが **5 を超える** 場合は `vertical-slice` スキルで並列可能な縦スライスに分割し、スタックを複数に割る（stacked-pr のアンチパターン「スタックの深さが6以上」を避ける）
3. 各スタックにブランチ名を割り当てる（stacked-pr スキルの命名規則に準拠）：
   ```
   feature/{EPIC-ID}-1-{short-name}   ← base: main（スタック先頭）
   feature/{EPIC-ID}-2-{short-name}   ← base: feature/{EPIC-ID}-1-...
   feature/{EPIC-ID}-3-{short-name}   ← base: feature/{EPIC-ID}-2-...
   ```
4. 各PRの「📚 Stacked PRs」セクションのテンプレートを生成する

**生成する `docs/06_wbs/stacked-pr-plan.md`**（テンプレートは `references/stacked-pr-plan-template.md` 参照）：

- 全体俯瞰図（スタック構造をツリーまたはリスト表示）
- スタンドアローンPR一覧（スタック外の独立チケット）
- 各スタックの詳細（ブランチ名・base・対応チケット・PR説明文テンプレート）

---

### Step 5: 受け入れテストケース生成

ここでの目的は「**システムが約束した価値を外から証明するテストケース群**」を作ること。
チケットドラフトの draft AC（実装完了条件）とは以下のように役割が異なる：

| | 生成タイミング | 視点 | 用途 |
|---|---|---|---|
| **AT ケース（このステップ）** | Step 5（設計完了直後） | ブラックボックス・価値レベル | リリース可否の判断基準 / WBS P4 の実行対象 |
| **ticket draft AC** | Step 4b（チケットドラフト） | チケット単位の実装完了条件 | kouunryuusui T0 で refine-ticket が補完 |

#### 入力

`acceptance-test-generator` スキルに以下を渡す：

- `docs/00_design-policy/設計方針書.md`（提供価値・スコープ・スコープ外）
- `docs/01_requirements/要件定義書.md`（ビジネスルール・ユースケース）
- `docs/02_basic-design/基本設計書.md`（外部インターフェース・API 定義）
- `docs/06_wbs/ticket-drafts/` 内のチケット一覧（AT ケース→チケット番号マッピング用）

```
# acceptance-test-generator スキル呼び出し
Skill(acceptance-test-generator)
# 上記ドキュメントを提供価値・仕様書として渡し、
# 8 観点のブラックボックステストケースを生成させる
```
...
| Tier | 出力 |
|------|------|
| **Tier 1** | `docs/05_acceptance-tests/AT-overview.md` 1ファイルのみ（軽量） |
| **Tier 2** | AT-overview.md + `features/` に主要機能単位のファイル |
| **Tier 3** | AT-overview.md + `features/` に機能別ファイル（エピック単位で分割） |

**`AT-overview.md` に含める内容**：

1. テスト全体のサマリー（機能一覧・テスト観点カバレッジ）
2. **AT ケース → チケット番号のマッピング表**（TC-ID と TICKET-NNN の対応）
3. P1 ケース一覧（リリース可否判断に必須なもの）

#### WBSとの連携

AT ケース生成後、以下を更新する：

- **WBS P0「要件精緻化」**: AT-overview.md を AC品質チェックの参照元として明記する
- **WBS P4「テスト」**: AT ケース群を実行対象として明記する
- **ticket-drafts の補完**: 各 `TICKET-NNN.md` に「関連ATケース」フィールドとして TC-ID を追記する
...
✅ Stacked PR 計画       → docs/06_wbs/stacked-pr-plan.md（Tier 2/3 のみ）
✅ 受け入れテストケース   → docs/05_acceptance-tests/ (AT-overview.md + features/ N件)

次のアクション：
1. WBSのP0（要件精緻化）から着手
   └── AT-overview.md を AC品質チェックの参照元として使う
...
4. WBSのP4（テスト）では docs/05_acceptance-tests/ のATケースを実行対象とする
5. PR作成時は stacked-pr-plan.md を参照しながら /stacked-pr スキルに従って実行
   └── rebase・force-with-lease・merge 順序は /stacked-pr スキルが提供する
```
...
| `docs/05_acceptance-tests/AT-overview.md` | P0のAC品質チェック参照元・P4の実行対象。ticket-drafts の draft AC（実装完了条件）とは別物 |
...
- **So What?/Why So? は対で往復させる**: So What?だけで止まると「機能が増える」止まりになる。Why So?で要件の文言に紐付けて初めて判断根拠になる
- **犠牲にするものが書けないときは再問い**: プラス面しか出てこない設計判断は見落としのサイン。So What?をもう一往復させる
...
- **Stacked PR 計画はDAGとして設計する**: チケットの依存関係は線形チェーンとは限らない。独立チケットをスタックに無理に含めると diff が無意味に膨らむ
- **スタックの深さは5以内**: 6本以上になりそうなら `vertical-slice` で縦スライスに分割し、スタックを複数に割る
- **Tier 1 にスタックは不要**: チケット3件以下の場合は1PRで完結させる。無理にスタックを作ると管理コストが実装コストを超える
- **Stacked PR 計画は計画だけ**: rebase・merge 手順・force-push のルールは `/stacked-pr` スキルに書かれている。SKILL.md 側でコマンドを複製しない
- **AT ケースと ticket draft AC を混同しない**: AT ケース（Step 5）は「システムが価値を届けるか」の feature-level 証明。ticket draft AC（Step 4b）は「このチケットの実装が完了したか」の条件。後者は refine-ticket が補完するため、AT ケースに同じ内容を重複して書かない
- **AT ケース生成は `acceptance-test-generator` に完全委任する**: 8観点・優先度定義・出力フォーマットはそちらに書かれている。SKILL.md 側で複製しない（両スキルが独立して進化できるように）
- **AT ケース生成後に ticket-drafts を補完する**: 各 TICKET-NNN.md に「関連ATケース」として TC-ID を追記しないと、kouunryuusui T0 がどのATケースが対応するか分からなくなる
- **Tier 1 で features/ を作らない**: AT-overview.md 1ファイルで十分。構造を作ること自体がコストになる

---

## 参照ファイル

- `references/so-what-why-so.md` — So What?/Why So? フレームワークの定義・問い文例・浅い vs 深い対比（Step 1・2 で参照）
...
- `references/stacked-pr-plan-template.md` — Stacked PR 計画書のテンプレート（Step 4c 出力形式）
- `acceptance-test-generator` スキル — Step 5 で呼び出す。8観点・TC-ID命名・出力フォーマット・Gotchas はすべてそちらを参照
