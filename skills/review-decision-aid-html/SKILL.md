...
3. **SHOULD セクション** — 各指摘をカード形式で。id/title/file/詳細(折りたたみ)/ラジオ選択肢。デフォルト「今回対応」
4. **NICE セクション** — 同構造。デフォルト「スキップ」。選択肢: `スキップ / 今回対応 / 後続化`
5. **DISCUSS セクション** — 同構造 + 必要に応じてSVG図 + 確認コメント案(折りたたみ)。デフォルト「レビュアー確認」。選択肢: `レビュアー確認 / 今回対応 / 対応不要返信`
6. **DEFER セクション** — 同構造 + ロードマップSVG。デフォルト「後続化」。選択肢: `後続化 / 今回対応 / 対応不要`
